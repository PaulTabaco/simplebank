postgres: 
	docker run --name postgres --network simplebank-network -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=mysecretpassword -d postgres:alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go parus.i234.me/paultabaco/simplebank/db/sqlc Store
